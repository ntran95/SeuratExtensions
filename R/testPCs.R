...
testPCs <- function(seurat_obj, from_to = 5:10,
  include_conditions = FALSE, n_cores = 10) {
    ...
    parallel::mclapply(PCs, function(i){
      ...
      seurat_obj <- RunUMAP(seurat_obj, umap.method = "umap-learn", dims = dims)
      ...
      umap_clusters <- umap_clusters + cleanUMAP()

      png(figurePath(paste0("exploratory-PCs/UMAP_Clusters",
        length(dims),".png")), width = 14, height = 12, units = "in", res = 200)
      print(umap_clusters)
      dev.off()
